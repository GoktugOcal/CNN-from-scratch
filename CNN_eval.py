          ...
          for j2 in range(w1):
            for f2 in range(f1):
              ...
              if temp_matrix[i2, j2, f2] == amax[f2]:
                d_L_d_input[h_start+ i2, w_start+ j2, f2] = d_L_d_out[h, w, f2]
...
# Set trained weights to layers
conv.filters = np.load("weights/conv1_filters.npy")
conv.biases = np.load("weights/conv1_biases.npy")

conv2.filters = np.load("weights/conv2_filters.npy")
conv2.biases = np.load("weights/conv2_biases.npy")

fc1.weights = np.load("weights/fc1_weights.npy")
fc1.biases = np.load("weights/fc1_biases.npy")

linear.weights = np.load("weights/fc2_weights.npy")
linear.biases = np.load("weights/fc2_biases.npy")
